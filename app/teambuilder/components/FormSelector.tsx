...
const FormSelector = ({
    ...
    const [displayValue, setDisplayValue] = useState(pokemonSpecies?.varieties[0].pokemon.name || '');
    ...
    const handleVarietyChange = (index: number) => {
        setVariety(index);
        setDisplayValue(formatName(pokemonSpecies?.varieties[index].pokemon.name || ''));
    };

    useEffect(() => {
        setDisplayValue(formatName(pokemonSpecies?.varieties[0].pokemon.name || ''));
    }, [pokemonSpecies]);
            ...
            <Listbox value={variety} onChange={handleVarietyChange}>
                {({ open }) => (
                    ...
                    <Listbox.Button className="relative group w-40 py-2 pl-3 pr-10 text-left bg-white dark:bg-gray-900 rounded-lg cursor-default hover:shadow-md sm:text-sm">
                        <span className="block capitalize truncate">{displayValue}</span>
