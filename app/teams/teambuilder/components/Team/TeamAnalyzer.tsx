...
const TeamAnalyzer: React.FC<TeamAnalyzerProps> = ({ team }) => {
  ...
  useEffect(() => {
    ...
    const initialMultipliers = pokemonTypes.reduce(
      (acc, curr) => ({ ...acc, [curr]: 1 }),
      {},
    );
  ...
  useEffect(() => {
    const hasLevitate = team.some((pokemon) =>
      pokemon.abilities.some(
        (ability: AbilityElement) => ability.ability.name === "levitate",
      ),
    );
  ...
  const generateWeaknessText = useCallback(
    (multiplier: number, type: string) => {
      if (multiplier > 1 && multiplier <= 2) {
        return (
          <span className="bg-orange-100/70 dark:bg-yellow-500/40 rounded-xl whit px-4 py-2 font-semibold text-yellow-500 dark:text-yellow-300 whitespace-nowrap">{`Weak to ${type.toUpperCase()}`}</span>
        );
      } else if (multiplier > 2) {
        return (
          <span className="bg-red-100/70 dark:bg-red-500/40 rounded-xl px-4 py-2 font-bold text-red-500 dark:text-red-300 whitespace-nowrap">{`Very weak to ${type.toUpperCase()}`}</span>
        );
      }
      return null;
    },
        ...
        <div className="flex flex-col 2xl:w-96 gap-4">
          <div className="flex flex-wrap gap-4">
            {Object.entries(multipliers)
              ...
              .map(([type, multiplier]) => (
                <div className="flex justify-center my-1" key={type}>
